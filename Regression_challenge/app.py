...
sheet = client.open("leaderboard").worksheet("F25_Regression_challenge")  # Open sheet named 'abc' in the 'leaderboard' spreadsheet
...
st.write("""Upload your prediction CSV with two columns: ID and Label.  
Ensure the file includes all required IDs, without any missing IDs.  
The leaderboard ranks submissions by **lowest Mean Squared Error (MSE)**.""")
    ...
    try:
        ...
        if leaderboard_data:
            ...
            leaderboard_placeholder.write(
                leaderboard_df.style.background_gradient(cmap="Reds_r").highlight_min(subset=['Score'], color='lightgreen')
            )
        else:
    ...
    if delimiter:
        try:
            ...
            if set(['ID', 'Label']).issubset(df.columns):
                if df['ID'].dtype == 'int64' or df['ID'].dtype == 'float64':
                    ...
                    if list(df['ID']) != list(correct_labels['ID']):
                        ...
                        # Calculate MSE
                        mse = mean_squared_error(merged_df['Label_pred'], merged_df['Label_true'])
                        st.write(f"Prediction Mean Squared Error (MSE): **{mse:.4f}**")
                        ...
                        if st.button("Add to Leaderboard"):
                            if user_name:
                                ...
                                score_entry = [user_name, mse, timestamp]

                                # Append new score to Google Sheets (always a new row)
                    ...
                    st.error("'ID' column must be numeric.")
            else:
